...
const getEventsByCategoryId = async (id) => {
  ...
  //  const res = await fetch(`${import.meta.env.LOCAL_URL}/api/events/category/${id}`);
  if (res.status === 200) {
    events.value = await res.json();
  } else console.log("Error, cannot get data");
};
const getPastEvents = async (isotime) => {
  ...
  //  const res = await fetch(`${import.meta.env.LOCAL_URL}/api/events/past/${isotime}`)
  if (res.status === 200) {
    events.value = await res.json();
  } else console.log("Error, cannot get data");
};
const getUpcomingEvents = async (isotime) => {
  ...
  //  const res = await fetch(`${import.meta.env.LOCAL_URL}/api/events/upcoming/${isotime}`)
  if (res.status === 200) {
    events.value = await res.json();
  } else console.log("Error, cannot get data");
};
...
const removeEvent = async (removeEventId) => {
  ...
  if (confirmDelete.value) {
    const res = await fetch(
      ...
      //  const res = await fetch(
      //  `${import.meta.env.LOCAL_URL}/api/events/${removeEventId}`,
      {
        method: "DELETE",
      }
    );
...
const editEvent = async (editingEvent) => {
  const res = await fetch(
    ...
    // const res = await fetch(
    //   `${import.meta.env.LOCAL_URL}/api/events/${editingEvent.id}`,
    {
      method: "PUT",
      headers: {
